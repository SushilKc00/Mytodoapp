...
export default function Home() {
  ...
  const getWorks = async () => {
    if (JSON.parse(localStorage.getItem("userid"))) {
      const data = await fetch(
        `http://localhost:8800/todo/get/${JSON.parse(
  ...
  const handleDelete = async (e) => {
    setLoading(true);
    const data = await fetch(`http://localhost:8800/todo/delete/${e}`, {
      method: "post",
    });
    const res = await data.json();
    if (res.success) {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    closeModal();
    const data = await fetch(`http://localhost:8800/todo/update/${updateId}`, {
      method: "post",
      headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!value) {
      ...
      setLoading(true);
      const data = await fetch("http://localhost:8800/todo/create", {
        method: "post",
        headers: {
          "Content-Type": "application/json",
        },
